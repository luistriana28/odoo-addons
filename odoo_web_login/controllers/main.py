...
class LoginHome(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        request.params['disable_footer'] = ast.literal_eval(
            param_obj.get_param(
                'login_form_disable_footer')) or False
        request.params['disable_database_manager'] = ast.literal_eval(
            param_obj.get_param(
                'login_form_disable_database_manager')) or False
        ...
        tz = (
            config_login_timezone and
            pytz.timezone(config_login_timezone) or
            pytz.utc)
        current_hour = datetime.datetime.now(tz=tz).hour or 10
        if ((current_hour >= 0 and current_hour < 3) or (
                current_hour >= 18 and current_hour < 24)):  # Night
            request.params['background_src'] = param_obj.get_param(
